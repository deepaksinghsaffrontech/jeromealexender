...
class Save extends \Saffron\Testimonial\Controller\Adminhtml\Testimo {
	/**
	...
	public function execute() {
		if ($data = $this->getRequest()->getPostValue()) {
			$model = $this->_objectManager->create('Saffron\Testimonial\Model\Testimo');
			...
			try {
				...
				$result = $uploader->save($mediaDirectory->getAbsolutePath(\Saffron\Testimonial\Model\Testimo::BASE_MEDIA_PATH));
				$data['avatar'] = \Saffron\Testimonial\Model\Testimo::BASE_MEDIA_PATH . $result['file'];
			} catch (\Exception $e) {
				if ($e->getCode() == 0) {
